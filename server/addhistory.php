...
//Set the maxID value for inserts
$maxID=0;
$sql="SELECT * FROM `gl_history` order by `HistoryID` DESC Limit 2";
...
array(1) {
  ...
  /* */
	if ($conn->query($sql.$sql2) === TRUE) {
		...
		//echo "<a href='".basename(__FILE__)."'>Refresh page</a><br>";
		//echo $sql.$sql2 . "<br>";
...
<br/>
<span>   <a href="<?php echo $_SERVER['PHP_SELF'];?>">Manual Entry</a></span>
<span> | <a href="<?php echo $_SERVER['PHP_SELF'];?>?mode=steam">Steam API</a></span>
...
<?php
	...
	foreach($resultarray['response']['games'] as $row){
		...
		//if (isset($lastrecord[$thisgamedata['Game_ID']]['Time'])) {
		...
		if(isset($steamindex[$row['appid']])){
			...
			<td class="Text">
				...
				<a href='viewgame.php?id=<?php echo $thisgamedata['Game_ID']; ?>' target='_blank'><?php echo $thisgamedata['Title']; ?></a>
				<span style='font-size: 70%;'>(<a href='http://store.steampowered.com/app/<?php echo $row['appid']; ?>' target='_blank'>Store</a>)</span>
			</td>
			<?php
		} else {
			...
			?>
			<td class="Text">&nbsp;&nbsp;&nbsp;MISSING: <a href='http://store.steampowered.com/app/<?php echo $row['appid']; ?>' target='_blank'><?php echo $gamename; ?></a></td>
			<?php
			$missing_count++;
...
<table class="ui-widget">
	...
	<td>
		...
		//<input type="checkbox" name="datarow[1][update]" CHECKED > 
		?>

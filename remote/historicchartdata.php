...
foreach ($dateData as $key => &$row) {
	...
	$row['AvgDayinYear']=$dateData[$row['yearstamp']]['YearTime']/(date("z", mktime(0,0,0,12,31,date("Y",$row['Date']))) + 1);
...
?>
	<tr class='dayclass <?php
	if(isset($row['WeekTime'])){echo "weekclass ";}
	if(isset($row['MonthTime'])){echo "monthclass ";}
	if(isset($row['YearTime'])){echo "yearclass ";}
	?>'>
	<td class='hidden numeric'><?php echo $row['Sort']; ?></td>
	<td class='numeric'><?php echo date("M/d/Y",$row['Date']); ?></td>
	<td class='numeric'><?php echo timeduration($row['Time'],"seconds"); ?></td>
	<td class='text'><?php echo date("l",$row['Date']); ?></td>
	<td class='numeric'><?php echo timeduration($weekday[date("l",$row['Date'])]['Avg'],"seconds"); ?></td>
	<td class='numeric'><?php echo timeduration($row['7avg']['Avg'],"seconds"); ?></td>
	<td class='numeric'><?php echo timeduration($row['30avg']['Avg'],"seconds"); ?></td>
<?php
	...
	//echo "<td>".date("w",$row['Date'])."</td>";
	if(isset($row['WeekTime'])){
		?>
		<td class='numeric'><?php echo timeduration($row['WeekTime'],"seconds"); ?></td>
		<td class='hidden numeric'><?php echo timeduration($weekavg['Avg'],"seconds"); ?></td>
		<?php
		if($row['weekstamp']==$lastweek){
			$dayofweek=(date("w",$lastdate)); ?>
			<td class='numeric'><?php echo timeduration(($row['WeekTime']/($dayofweek+1))*(7),"seconds"); ?></td>
			<td class='numeric'><?php echo timeduration($row['WeekTime']/($dayofweek+1),"seconds"); ?></td>
		<?php } else { ?>
			<td class='numeric'><?php echo timeduration(($row['WeekTime']/$row['WeekDayCount'])*7,"seconds"); ?></td>
			<td class='numeric'><?php echo timeduration($row['WeekTime']/$row['WeekDayCount'],"seconds"); ?></td>
		<?php }


		//echo"<br>timeduration("; var_dump($dateData[$row['monthstamp']]['MonthTime']); echo " / date(\"t\","; var_dump($row['Date']); echo " ))<br>";
		//echo"timeduration("; var_dump($dateData[$row['monthstamp']]['MonthTime']); echo " / "; var_dump(date("t",$row['Date'])); echo " )<br>";
		//echo"timeduration("; var_dump($dateData[$row['monthstamp']]['MonthTime'] / date("t",$row['Date'])); echo " )<br>";
		//var_dump(timeduration($dateData[$row['monthstamp']]['MonthTime'] / date("t",$row['Date']))); echo "/60/60*7<br>";
		//var_dump(strtotime(timeduration($dateData[$row['monthstamp']]['MonthTime'] / date("t",$row['Date'])))/60/60*7); echo "<br>";
		//var_dump(timeduration($dateData[$row['monthstamp']]['MonthTime'] / date("t",$row['Date'])/60/60*7)); echo "<br>";
	...
	<?php }
		//DEBUG ROWS
		//echo "<td class='text'>".date("F",$row['monthstamp'])."</td>";
		//$monthstamp=strtotime((date("Y",$row['Date']))."-".(date("m",$row['Date']))."-1");
		//echo "<td class='text'>".date("Y-m-d",$monthstamp)."</td>"; //Show date of monthstamp
		//echo "<td class='text'>".date("Y-m-d",$row['monthstamp'])."</td>"; //Show date of monthstamp
	if(isset($row['MonthTime'])){
		?>
		<td class='hidden text'><?php echo date("F",$row['Date']); ?></td>
		<td class='numeric'><?php echo timeduration($row['MonthTime'],"seconds"); ?></td>
		<td class='hidden numeric'><?php timeduration($row['MonthTime']/date("t",$row['Date']),"seconds"); ?></td>
		<?php
		if($row['monthstamp']==$lastmonth){
			$dayofmonth=(date("j",$lastdate));
			$daysinmonth=(date("t",$lastdate));
			?>
			<td class='numeric'><?php echo timeduration(($row['MonthTime']/($dayofmonth))*($daysinmonth-$dayofmonth),"seconds"); ?></td>
			<td class='numeric'><?php echo timeduration($row['MonthTime']/($dayofmonth),"seconds"); ?></td>
			<?php
		} else {
			//DEBUG ROW
			/*
			echo "<td class='numeric'>";
			//echo timeduration($row['MonthTime'],"seconds");
			echo "/";
			echo $row['MonthDayCount'];
			echo "*";
			echo date("t",$row['monthstamp']);
			echo "=";
			echo timeduration(($row['MonthTime']/$row['MonthDayCount'])*date("t",$row['monthstamp']),"seconds");
			echo "</td>";
			*/
			?>
			<td class='numeric'><?php echo timeduration(($row['MonthTime']/$row['MonthDayCount'])*date("t",$row['monthstamp']),"seconds"); ?></td>
			<td class='numeric'><?php echo timeduration($row['MonthTime']/$row['MonthDayCount'],"seconds"); ?></td>
			<?php
		}
		...
		<td></td>
	<?php } ?>
	<td class='hidden numeric'><?php echo timeduration($dateData[$row['monthstamp']]['MonthTime']/date("t",$row['Date']),"seconds"); ?></td>
	<?php
	if(isset($row['YearTime'])){
		?>
		<td class='hidden numeric'><?php echo date("Y",$row['Date']); ?></td>
		<td class='numeric'><?php echo timeduration($row['YearTime'],"seconds"); ?></td>
		<?php
		if($row['yearstamp']==$lastyear){
			$dayofyear=(date("z",$lastdate)+1);
			$daysinyear=365+(date("L",$lastdate));
			?>
			<td class='numeric'><?php echo timeduration(($row['YearTime']/($dayofyear))*($daysinyear-$dayofyear),"seconds"); ?></td>
			<td class='numeric'><?php echo timeduration($row['YearTime']/($dayofyear),"seconds"); ?></td>
		<?php } else { ?>
			<td class='numeric'><?php echo timeduration($row['YearTime']/$row['YearDayCount']*(365+(date("L",$row['yearstamp']))),"seconds"); ?></td>
			<td class='numeric'><?php echo timeduration($row['YearTime']/$row['YearDayCount'],"seconds"); ?></td>
			<td class='hidden numeric'><?php echo timeduration((($row['YearTime']/(365+(date("L",$row['yearstamp']))))),"seconds"); ?></td>
		<?php }
		...
		<td></td>
	<?php }
	//echo "<td class='numeric'>".timeduration($row['AvgDayinYear'],"seconds")."</td>";

	//echo "<td>".$row['7avg']['Debug']."</td>";
	//echo "<td>".date("M/d/Y h:i:s A",$row['Date'])."</td>";
	//echo "<td>";	var_dump($row);	echo "</td>";
	//echo "<td>" .timeduration($currentmonthtime,"seconds" )."</td>";
	
	/*
	echo "<td>".date("Y-m-d",$row['monthstamp'])." ";
	if(isset($row['monthstamp'])){
		echo timeduration($dateData[$row['monthstamp']]['MonthTime'],"seconds" );
	} else {
		echo "No monthstamp";
	}
	echo "</td>";
	*/
	...
	<tr class="hidden"><td colspan=10><?php echo var_dump($row); ?></td></tr>
<?php } ?>
</tbody>
</table>
<?php
...
/* */
	//echo "<br>";echo $GoogleChartDataString['AllMonths'];
...
function drawCurveTypes() {
  ...
  data.addRows([
	<?php echo $GoogleChartDataString['TwoWeeks']; ?>
  ]);
  ...
  data2.addRows([
	<?php echo $GoogleChartDataString['AllWeeks']; ?>
  ]);
 ...
 //data3.addColumn('number', 'Average Month');
  ...
  data3.addRows([
	<?php echo $GoogleChartDataString['AllMonths']; ?>
  ]);
  ...
  data4.addRows([
	<?php echo $GoogleChartDataString['Years']; ?>
  ]);
